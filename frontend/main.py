...
def fetch_qa_data(topic, set_id):
    response = requests.get(f"http://backend:8000/get_qa_data/{topic}/{set_id}")
...
def Knowledge_base_QA():
    ...
    question = next((entry for entry in qa_data if entry["question"] == selected_question), None)
    if question:
...
def Answer_by_summary():
    ...
    summary = fetch_summary_data(selected_topic)
    st.write(f"{selected_topic} summary:")
    # Logic to find answer by summary
    if summary:
        st.write(summary)
